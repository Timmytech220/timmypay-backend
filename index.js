...
// --- CONFIGURATION ---
const MONNIFY_API_KEY = process.env.MONNIFY_API_KEY || "MK_TEST_F7W9X95BGN";
const MONNIFY_SECRET_KEY = process.env.MONNIFY_SECRET_KEY || "2KP8NZSQRVNTZYB7ET9MCANS9PF6GQ8N";
...
app.get(["/get-account", "/get-account/"], async (req, res) => {
    ...
    try {
        ...
        if (userData && userData.virtualAccount && userData.virtualAccount.accountNumber) {
            return res.json({ success: true, ...userData.virtualAccount });
        }

        // Logic for Monnify generation
        res.json({ success: true, message: "Add your Monnify API call here" });
    } catch (error) {
